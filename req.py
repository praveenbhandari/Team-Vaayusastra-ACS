# Adafruit-Blinka==6.17.0
# adafruit-circuitpython-ads1x15==2.2.12
# adafruit-circuitpython-bmp3xx==1.3.7
# adafruit-circuitpython-bno08x==1.1.2
# adafruit-circuitpython-busdevice==5.1.1
# adafruit-circuitpython-framebuf==1.4.8
# adafruit-circuitpython-rfm9x==2.2.2
adafruit-extended-bus==1.0.2
Adafruit-GPIO==1.0.3
# Adafruit-Libraries==0.1
# Adafruit-PlatformDetect==3.18.0
# Adafruit-PureIO==1.1.9
# adafruit-python-shell==1.3.3
# appdirs==1.4.3
# args==0.1.0
# asn1crypto==0.24.0
# astroid==2.1.0
# asttokens==1.1.13
# automationhat==0.2.0
# beautifulsoup4==4.7.1
# blinker==1.4
# blinkt==0.1.2
# buttonshim==0.0.2
# Cap1xxx==0.1.3
# certifi==2018.8.24
# chardet==3.0.4
# Click==7.0
# clint==0.5.1
# colorama==0.3.7
# colorzero==1.1
# cookies==2.2.1
# cryptography==2.6.1
# cupshelpers==1.0
# cycler==0.10.0
# decorator==4.3.0
# digi-xbee==1.4.1
# docutils==0.14
# drumhat==0.1.0
# entrypoints==0.3
# envirophat==1.0.0
# ExplorerHAT==0.4.2
# FaBo9Axis-MPU9250==1.0.0
# Flask==1.0.2
# fourletterphat==0.1.0
# gpiozero==1.6.2
# guizero==0.6.0
# html5lib==1.0.1
# idna==2.6
imutils==0.5.4
ipykernel==4.9.0
ipython==5.8.0
ipython-genutils==0.2.0
# isort==4.3.4
# itsdangerous==0.24
# jedi==0.13.2
# Jinja2==2.10
# jupyter-client==5.2.3
# jupyter-core==4.4.0
# keyring==17.1.1
# keyrings.alt==3.1.1
# kiwisolver==1.0.1
# lazy-object-proxy==1.3.1
# linecache2==1.0.0
# logilab-common==1.4.2
# lxml==4.3.2
# MarkupSafe==1.1.0
# matplotlib==3.0.2
# mccabe==0.6.1
# microdotphat==0.2.1
# mote==0.0.4
# motephat==0.0.3
# mpu9250-jmdev==1.0.12
# mypy==0.670
# mypy-extensions==0.4.1
# nudatus==0.0.4
numpy==1.16.2
# oauthlib==2.1.0
# olefile==0.46
# pantilthat==0.0.7
# parso==0.3.1
# pbr==4.2.0
# pexpect==4.6.0
# pgzero==1.2
# phatbeat==0.1.1
# pianohat==0.1.0
picamera==1.13
# pickleshare==0.7.5
# picraft==1.0
# piglow==1.2.5
pigpio==1.78
Pillow==5.4.1
# prompt-toolkit==1.0.15
psutil==5.5.1
# pycairo==1.16.2
# pycodestyle==2.4.0
# pycrypto==2.6.1
# pycups==1.9.73
# pyflakes==2.0.0
pyftdi==0.53.3
# pygame==1.9.4.post1
# Pygments==2.3.1
# PyGObject==3.30.4
# pyinotify==0.9.6
# PyJWT==1.7.0
# pyliblo==0.10.0
# pylint==2.2.2
# pyLoRa==0.3.1
# pyOpenSSL==19.0.0
# pyparsing==2.2.0
# pyperclip==1.6.4
pyserial==3.4
pysmbc==1.0.15.6
# python-apt==1.8.4.3
# python-dateutil==2.7.3
pyusb==1.2.1
# pyxdg==0.25
# pyzmq==17.1.2
# qtconsole==4.3.1
# rainbowhat==0.1.0
# raspberrypi-esp32-i2c==0.0.6
# reportlab==3.5.13
# requests==2.21.0
# requests-oauthlib==1.0.0
# responses==0.9.0
# roman==2.0.0
# rpi-ws281x==4.3.1
RPi.GPIO==0.7.0
RTIMULib==7.2.1
# scrollphat==0.0.7
# scrollphathd==1.2.1
# SecretStorage==2.3.1
# semver==2.0.1
# sense-emu==1.2
# sense-hat==2.2.0
# simplegeneric==0.8.1
# simplejson==3.16.0
# six==1.12.0
# skywriter==0.0.7
smbus2==0.4.1
# sn3218==1.2.7
# soupsieve==1.8
# spidev==3.5
ssh-import-id==5.7
# sysv-ipc==1.1.0
# testresources==2.0.0
thonny==3.1.0
thonny-pi==1.1
# tornado==5.1.1
# touchphat==0.0.1
# traceback2==1.4.0
# traitlets==4.3.2
# twython==3.7.0
# typed-ast==1.3.1
# uflash==1.2.4
# unicornhathd==0.0.4
# unittest2==1.1.0
# urllib3==1.24.1
# wcwidth==0.1.7
# webencodings==0.5.1
# Werkzeug==0.14.1
# wrapt==1.10.11
# XBee==2.3.2
