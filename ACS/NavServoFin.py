...
def left_move(Xposition, Yposition):
    ...
    position2 = 1. / 18. * (j) + 2
    lpwm.ChangeDutyCycle(position)
    rpwm.ChangeDutyCycle(position2)
    # ser.write((str(Xposition) + 'a').encode('utf-8'))
    # time.sleep(0.03)
    Yposition += 10  # RIGHT SERVO
    # ser.write((str(Yposition) + 'a').encode('utf-8'))
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    for cnt in contours:
        ...
        # ///////////////////////////////////////////////////////////////
        if x > int(frame.shape[1] / 2) and y > int(frame.shape[0] / 2) and x > int(frame.shape[1] / 2) + 30 and y > int(
                frame.shape[0] / 2) + 30 and x < int(frame.shape[1] / 2) + 100 and y < int(frame.shape[0] / 2) + 100:
            cv2.putText(img, "Circle is towards bottom right!!", (75, 105), cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 255, 0),
                        2)
        ...
        elif x < int(frame.shape[1] / 2) and y > int(frame.shape[0] / 2) and x < int(
                frame.shape[1] / 2) + 30 and y > int(frame.shape[0] / 2) + 30 and x < int(
                frame.shape[1] / 2) + 35 and y > int(frame.shape[0] / 2) + 30:
            cv2.putText(img, "Circle is towards bottom left !!", (75, 105), cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 255, 0),
                        2)
        ...
        #     cv2.putText(img, "Circle is towards extreme bottom left !!", (75, 105), cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 255, 0), 2)
                ...
                frame.shape[1] / 2) + 30 and y < int(frame.shape[0] / 2) + 30:
            cv2.putText(img, "Circle is towards top left !!", (75, 105), cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 255, 0), 2)
            left_move(Xposition, Yposition)
        # elif x<int(frame.shape[1] / 2) and y<int(frame.shape[0] / 2)and x<int(frame.shape[1] / 2)+35 and y<int(frame.shape[0]/2)+35:
        #     cv2.putText(img, "Circle is towards extreme top left !!", (75, 105), cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 255, 0), 2)
        elif x > int(frame.shape[1] / 2) and y < int(frame.shape[0] / 2) and x > int(
                frame.shape[1] / 2) + 30 and y < int(frame.shape[0] / 2) + 30:
            cv2.putText(img, "Circle is towards top right !!", (75, 105), cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 255, 0), 2)
            right_move(Xposition, Yposition)
        # elif x>int(frame.shape[1] / 2) and y<int(frame.shape[0] / 2)and x>int(frame.shape[1] / 2)+35 and y<int(frame.shape[0]/2)+35:
